...
> 状态说明：这是 Taotie 设计稿。当前代码已实现扫描、启发式/LLM 分类、黑名单、入库队列、历史记录和 watcher 配置管理；尚未实现独立常驻后台 watcher daemon。`.docx` 可被扫描与预览分类，但正文入库提取尚未接入 `gmind ingest`。
...
### 6.3 Swift 端（重写 `TaotieView.swift`）
