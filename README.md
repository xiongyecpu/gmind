...
GMind 是一个为知识工作者设计的**个人知识库 CLI 工具**。它把碎片化的笔记、阅读、思考，通过向量搜索和知识图谱串联成一张可查询、可探索的知识网络。

核心理念：
- **写即入库** — 一条命令把想法写入数据库，自动向量化
- **问即搜索** — 自然语言查询，向量语义召回 + LLM 总结
- **多机同步** — 家里、公司多台电脑，draft / published 状态自动合并
...
| 方案 | v1 多 Schema | **v2 单库 + status** ✅ |
|------|-------------|------------------------|
| 维护成本 | N 个 schema 的迁移 | 一张表，两列解决 |
| 跨机查询 | 需要 sync 到 public | 天然实时，全局可见 |
| 同步逻辑 | 推/拉双向同步 | draft → published，代码量砍 1/3 |
...
```sql
-- 核心字段
origin_node TEXT  -- 物理节点（home/office）
